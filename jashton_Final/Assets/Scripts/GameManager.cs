...
{	
	...
	void OnGUI() {
		...
		if(isFinished)
		{
			GUI.Box(new Rect(Screen.width / 2 - 65, 115, 130, 40), "Your Time Was");
			GUI.Label(new Rect(Screen.width / 2 - 10, 130, 20, 30), ((int)elapsedTime).ToString());
		}
		else if(isRunning)
		{ 
			// If the game is running, show the current time
			GUI.Box(new Rect(Screen.width / 2 - 65, Screen.height - 115, 130, 40), "RACE TIME");
